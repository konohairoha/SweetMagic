...
public class TileMFAccelerator extends TileMFBase {
	...
	private static final List<Block> blockList = Arrays.<Block> asList(
		Blocks.AIR, BlockInit.pedestal_creat, BlockInit.magia_accelerator, BlockInit.gravity_chest
	);
	...
	public void update() {
		...
		super.update();
		if (this.getTime() % 2 != 0 || !this.canUpdate() || !this.isActive(this.pos)) { return; }

		// 座標リストの取得
		Iterable<BlockPos> posList = this.getPosList(this.pos.add(-RANGE, 0, -RANGE), this.pos.add(RANGE, RANGE, RANGE));
		int sumMF = 0;
		...
		for (BlockPos p : posList) {
			...
			TileEntity tile = this.getTile(p);
