...
public class TileToolRepair extends TileMFBase {
	...
	// 杖スロット
	public final ItemStackHandler toolInv = new InventoryWoodChest(this, this.getInvSize()) {
	...
	public void update() {
		...
		// 一定時間経てば処理
		if (this.getTime() % 20 != 0 || this.isMfEmpty() || !this.isActive(this.pos)) { return; }
	...
	public void checkRangePlayer () {
		this.findPlayer = this.findRangePlayer(16D, 8D, 16D);
	}
	...
	public void repairTool () {
		...
		for (int i = 0; i < this.getTool().getSlots(); i++) {
			...
			int useMF = Math.min(75000, (int) ((maxDamage * 0.1F) * (maxDamage * 0.1F) * 0.1F));
