...
public class RenderLightMagic<T extends Entity> extends Render<T> {
	...
	public void renderBullet(T entity, double x, double y, double z, float entityYaw, float parTick) {
		this.bindEntityTexture(entity);
		GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
		GlStateManager.pushMatrix();
		GlStateManager.disableLighting();
		GlStateManager.translate((float) x, (float) y, (float) z);
        GlStateManager.rotate(entity.prevRotationYaw + (entity.rotationYaw - entity.prevRotationYaw) * parTick - 90.0F, 0.0F, 1.0F, 0.0F);
        GlStateManager.rotate(entity.prevRotationPitch + (entity.rotationPitch - entity.prevRotationPitch) * parTick, 0.0F, 0.0F, 1.0F);
        Tessellator tessellator = Tessellator.getInstance();
		BufferBuilder vertexbuffer = tessellator.getBuffer();
		int i = 0;
		float f = 0.0F;
		float f1 = 0.5F;
		float f2 = (i * 10F) / 32.0F;
		float f3 = (5F + i * 10F) / 32.0F;
		float f4 = 0.0F;
		float f5 = 0.15625F;
		float f6 = (5F + i * 10F) / 32.0F;
		float f7 = (10F + i * 10F) / 32.0F;
		float f8 = 0.15625F;
