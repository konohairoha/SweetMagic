...
public class GuiMagiaWrite extends GuiContainer {

	private static final ResourceLocation texture = new ResourceLocation(SweetMagicCore.MODID, "textures/gui/gui_magiawrite.png");
	private final TileMagiaWrite tile;

	public GuiMagiaWrite(InventoryPlayer invPlayer, TileMagiaWrite tile) {
		super(new ContainerMagiaWrite(invPlayer, tile));
		this.xSize = 176;
		this.ySize = 178;
		this.tile = tile;
	}
	...
	protected void drawGuiContainerBackgroundLayer(float var1, int var2, int var3) {
		...
		// 必要コストとゲージの表示
		if (!this.tile.getToolItem().isEmpty()) {

			int useMF = this.tile.getEnchantCost();

			if (useMF > 0) {

				// 進行度の表示
				int progress = this.tile.smeltTime;
				if (progress > 0) {
				...
				this.fontRenderer.drawString(I18n.format(tip, new Object[0]), x + 63, y + 42, 0x404040, true);
			}
		}
	}
	...
	protected void drawGuiContainerForegroundLayer(int mouseX, int mouseY) {
		...
		if (tip_x2 <= mouseX && mouseX <= tip_x2 + 5
			...
			float progress = this.tile.smeltTime * 2F;
