...
public class TileFreezer extends TileSMBase {
	...
	private final ItemStackHandler waterInventory = new InventoryWoodChest(this, 1);	// 水投入スロット
	private final ItemStackHandler iceInventory = new InventoryWoodChest(this, 2);		// 氷スロット
	private final ItemStackHandler handInventory = new InventoryWoodChest(this, 1);		// ハンドスロット
	private final ItemStackHandler inputInventory = new InventoryWoodChest(this, 6);	// 入力スロット
	private final ItemStackHandler outputInventory = new InventoryWoodChest(this, 4);	// 出力スロット
	private final ItemStackHandler chestInventory = new InventoryWoodChest(this, 104);	// チェストスロット
	...
	private final int cookMaxTime = 80;
	...
	public void update() {
		...
		if (this.isCooking) {
			...
			if (this.cookTime > this.cookMaxTime) {
				this.cookTime = this.cookMaxTime - 10;
			}
		}
		...
		if (this.tickTime % 20 == 0) {
			...
			// 作成中から5秒経ったら
			else if (this.isCooking && this.cookTime % this.cookMaxTime == 0) {
				this.finishCook();
			}
		}
	}
	...
	public void checkRecipe () {
		...
		if (this.isSever()) {
			...
			// リストをまとめる
			ItemHelper.compactStackList(results);
	...
	public int getCookProgress (int value) {
		return this.cookTime >= this.cookMaxTime ? value : (int) (value * this.cookTime / 100);
	}
