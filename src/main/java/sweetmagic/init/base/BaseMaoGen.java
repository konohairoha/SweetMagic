...
public class BaseMaoGen extends MapGenStructure {

    public int distance;
    public final SMChunkGen provider;

    public BaseMaoGen(SMChunkGen provider) {
        this.distance = 32;
    ...
    protected boolean canSpawnStructureAtCoords(int chunkX, int chunkZ) {
        ...
        Random random = this.world.setRandomSeed(pX, pZ, 14267312);
        pX = pX * this.distance;
        pZ = pZ * this.distance;
        pX = pX + random.nextInt(this.distance - 8);
        pZ = pZ + random.nextInt(this.distance - 8);
