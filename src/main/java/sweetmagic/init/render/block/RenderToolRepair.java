...
public class RenderToolRepair extends TileEntitySpecialRenderer<TileToolRepair> {
	...
	public void render(TileToolRepair te, double x, double y, double z, float partialTicks, int destroyStage, float alpha) {
		if (!te.isMfEmpty()) {
			this.rebderEffect(te, x, y, z, partialTicks);
		}
	}

	public void rebderEffect (TileToolRepair te, double x, double y, double z, float partialTicks) {
        this.renderItem(te, 0, x + 0.25, y + 0.125, z, partialTicks);
        this.renderItem(te, 1, x, y + 0.125, z + 0.25, partialTicks);
        this.renderItem(te, 2, x - 0.25, y + 0.125, z, partialTicks);
        this.renderItem(te, 3, x, y + 0.125, z - 0.25, partialTicks);
        this.renderAether(te, x, y + 0.025, z, partialTicks);
	}

	protected void renderItem(TileToolRepair te, int slot, double x, double y, double z, float partialTicks) {
