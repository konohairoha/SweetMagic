...
public class RenderOven extends TileEntitySpecialRenderer<TileFlourMill> {

	@Override
	public void render(TileFlourMill te, double x, double y, double z, float parTick, int stage, float alpha) {

		IBlockState state = te.getWorld().getBlockState(te.getPos());
		Block block = state.getBlock();
		if (!(block instanceof BlockOven) || ((BlockOven) block).getData() != 1) { return; }

		EnumCook cook = te.getCook(state);
		if (cook == null || cook.isOFF()) { return; }

		GlStateManager.pushMatrix();
		GlStateManager.color(1F, 1F, 1F, 1F);
		GlStateManager.translate(x + 0.5F, y + 0.45F, z + 0.5F);
		GlStateManager.rotate(RenderUtils.getFacingAngle(state) + 180F, 0F, 1F, 0F);
		this.renderItem(te, cook, x, y - 0.5, z, parTick);
		GlStateManager.popMatrix();
	}

	protected void renderItem(TileFlourMill te, EnumCook cook, double x, double y, double z, float parTick) {
