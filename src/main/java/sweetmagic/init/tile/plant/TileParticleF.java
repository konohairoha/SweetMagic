...
public class TileParticleF extends TileSMBase {
	...
	public void update() {
		...
		if (block == BlockInit.dm_plant) {
			...
			// 雨が降っていないなら
			else if (!this.world.isRaining() && !this.world.isRemote) {

				flagRaining = false;

				// 花が咲いてる状態なら
				if (MagiaFlower.getNowStateMeta_open(state) == 3) {
					this.world.setBlockState(this.pos, MagiaFlower.withStage(this.world, state, 2), 2);
				}
			}
		}
		...
		if (worldTime < 12000) {
			...
			// ムーンブロッサムの花
			else if (block == BlockInit.moonblossom_plant && !this.world.isRemote) {
		...
		// 夜だったら
			...
			// サニーフラワーの花
			else if (block == BlockInit.sannyflower_plant && !this.world.isRemote) {
				if(MagiaFlower.getNowStateMeta_open(state) == 3 && !flagDaytime) {
					this.world.setBlockState(this.pos, MagiaFlower.withStage(this.world, state, 2), 2);
				}
			}
	...
	private void spawnParticles(World world, BlockPos pos) {
		...
		for (int i = 0; i < 6; ++i) {
			...
			world.spawnParticle(EnumParticleTypes.VILLAGER_HAPPY, d1, d2, d3, 0.0D, 0.0D, 0.0D);
        }
    }
}
