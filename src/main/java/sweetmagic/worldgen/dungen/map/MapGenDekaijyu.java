...
public class MapGenDekaijyu extends BaseMaoGen {
    ...
    public List<Biome> getBiomeList () {
    	return Arrays.<Biome>asList(BiomeInit.ESTORFOREST);
    }

    protected StructureStart getStructureStart(int chunkX, int chunkZ) {
        return new MapGenDekaijyu.Start(this.world, this.provider, this.rand, chunkX, chunkZ);
    }
    ...
    public static class Start extends BaseStructureStart {
        ...
        public void create(World world, SMChunkGen chunk, Random rand, int chunkX, int chunkZ) {
            ...
            List<DekaijyuPiece.DekaijyuTemplate> list = Lists.<DekaijyuPiece.DekaijyuTemplate>newLinkedList();
            DekaijyuPiece.generateCore(world.getSaveHandler().getStructureTemplateManager(), pos, rot, list, rand);
				...
				for (int z = sbb.minZ; z <= sbb.maxZ; ++z) {
					for (int y = sbb.minY; y <= sbb.maxY; ++y) {
	                    ...
	                    if (block == Blocks.DIAMOND_BLOCK) {
	                    	world.setBlockState(pos, BlockInit.treasure_chest.getDefaultState().withProperty(BlockWoodChest.FACING, EnumFacing.EAST), 2);
	                    	this.setLootTable(world, rand, pos, LootTableList.CHESTS_END_CITY_TREASURE, 0.35F);
	                    }

	                    else if (block == Blocks.GOLD_BLOCK) {
	                    	world.setBlockState(pos, Blocks.CHEST.getDefaultState().withProperty(BlockChest.FACING, EnumFacing.EAST), 2);
	                    	this.setLootTable(world, rand, pos, LootTableInit.SMBOUNUS);
	                    }
